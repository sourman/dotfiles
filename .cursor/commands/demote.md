Move a file or MCP server entry from the global ~/.cursor directory to the project's .cursor directory.

For files:
1. Parse the input to determine the filename and folder type (commands, rules, plans, tools, or mcps)
2. Check if the file exists in ~/.cursor/{folder}/{filename}
3. If the file doesn't exist, check if it's already in .cursor/{folder}/ and inform the user
4. If the file exists in ~/.cursor, move it to .cursor/{folder}/{filename} in the current project
...
If the user provides just a filename, search in all supported folders (commands, rules, plans, tools, mcps) in ~/.cursor to find it.
If multiple matches are found, ask the user which one to demote.
If the input doesn't match any file but matches an MCP server name in ~/.cursor/mcp.json, treat it as an MCP demotion.